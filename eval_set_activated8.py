# Solve the last puzzle, given you learned according to real game in world1
...
for k in range(n_runs):
    for world in range(1):
        ...
        # solve tangram --> solution (don't actually solve)
            ...
            if act:
                # sol.set_activation(training_output)
                sol.set_activation(global_out_list[2*puzzle+1])
            sol.run_task(task, duration=300, stop=True)
            seq = sol.get_seq_of_moves_v2(task_all_pieces)
            # print(seq)
            # print ('game: ', game)
...
save = True
if save is True:
    with open('eval_set_activation8_30_runs_training.pkl', 'wb') as f:
