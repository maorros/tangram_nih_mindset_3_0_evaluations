# create seeds of solving each puzzle after learning puzzles from actual game 'w1'
...
# get seed for current puzzle in game
    print('iter:', iter, ' ref:', 'current' )
    training_task, training_input, training_output = json_to_NN(sgc.paths[path_list[iter]][puzzles_indx_list[iter]])
...
# get seed for last puzzle in game
    print('iter:', iter, ' ref:', 'last')
...
if save is True:
    if CONDITION == 'curious':
        with open('curious_y_output_e12_1e5.pkl', 'wb') as f:
    ...
    if CONDITION == 'not_curious':
        with open('not_curious_y_output_e12_1e5.pkl', 'wb') as f:
            pickle.dump([selection_sequence,training_set_input,training_set_output, global_out_list, global_H_list], f, pickle.HIGHEST_PROTOCOL)
