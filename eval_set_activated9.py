# Solve the current puzzle, given you learned according to real game in world1
# robot selections [1,1,2,1,2,1]
# solutions length [5,5,11,7,193,13]
# path_list [1,1,3,1,3,1]
# puzzle in path list [1,2,3,4,5,6]
...
for k in range(n_runs):
    for world in range(1):
        ...
        # solve tangram --> solution (don't actually solve)
            print('k:', k, 'puzzle:',iter)
            task = Task()
            task.create_from_json(sgc.paths[path_list[iter]][puzzles_indx_list[iter]])
            training_task, training_input, training_output = json_to_NN(sgc.paths[path_list[iter]][puzzles_indx_list[iter]])
            sol.set_initial_task(task)
            game = 1
            # if game > 0:
            # print('training_output', training_output)
            if act:
                # sol.set_activation(training_output)
                sol.set_activation(global_out_list[2*puzzle])
            sol.run_task(task, duration=300, stop=True)
            seq = sol.get_seq_of_moves_v2(task_all_pieces)
            # print(seq)
            # print ('game: ', game)
            ...
            if act:
                seq_lens_act_T.append(len(seq))
                lens_arr_act_T[k, iter] = len(seq)
            else:
                seq_lens_act_F.append(len(seq))
                lens_arr_act_F[k, iter] = len(seq)
...
save = True
if save is True:
    with open('eval_set_activation9_30_runs_training.pkl', 'wb') as f:
...
with open('eval_set_activation9_30_runs_training.pkl', 'rb') as f:  # Python 3: open(..., 'rb')
...
plt.title('Solving current puzzle in real game.')
